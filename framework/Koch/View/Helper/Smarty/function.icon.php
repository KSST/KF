...
function Smarty_function_icon($params)
{
    ...
    $src = realpath(APPLICATION_THEMES . 'core/images/' . $icondir . DIRECTORY_SEPARATOR . $name . '.png');

    // if we got no valid src, set a default image
    if (isset($src) and is_file($src) == false) {
        #$src = WWW_ROOT_THEMES_CORE . 'images/noimage.gif';
        $src = APPLICATION_THEMES . 'core/images/noimage.gif';
    ...
    $src = str_replace( APPLICATION_THEMES, WWW_ROOT_THEMES, $src );
    $src = str_replace( '\\', '/', $src );
